...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            pytest.raises(
                ValueError, match=r"y_true contains more than 2 different values."
            ),
        ),
        (
            np.array([0, 0, 1, 1]),
            2,
            pytest.raises(ValueError, match=r"pos_label is not contained in y_true."),
        ),
    ],
)
def test_precision_recall_curve(
    y_true: np.ndarray, pos_label: Optional[int], expectation: ContextManager
) -> None:
